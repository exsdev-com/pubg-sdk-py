"""Custom exceptions for pubg-sdk-py.
...
class RequestTimeoutError(Exception):
    ...
    code = "pubghq_client_request_timeout"

    def __init__(self, message: str = "Request to PUBG API has timed out") -> None:
...
class HTTPResponseError(Exception):
    ...
    code: str = "pubghq_client_response_error"
    status: int
    ...
    def __init__(self, response: httpx.Response, message: Optional[str] = None) -> None:
        if message is None:
            message = (
                f"Request to PUBG API failed with status: {response.status_code}"
            )
        super().__init__(message)
...
class APIErrorCode(str, Enum):
    ...
    """Given the bearer token used, the pubg_client doesn't have permission to
